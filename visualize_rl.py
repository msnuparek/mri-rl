...
def render_frame(
    ...
    """Vykreslí jeden frame epizody (vždy 4 panely: GT, ZF, Recon, Maska)."""
    ...
    axs[3].set_yticks([])
    axs[3].set_title(f"step {step}  |  budget {budget_left}")
...
def main():
    ...
    if use_pseudo_ref:
        print(f"[info] Pseudo-reference mode: ACS={pseudo_ref_acs}")
    ...
    while not done:
        ...
        frames.append(render_frame(gt_mag, zf_mag, pr_mag, mask_np, step, budget_left))
